...
func main() {
	...
	if len(os.Args) == 1 {
		...
		if len(os.Args) >= 3 {
			...
			genesis := Block{"genesis", Biddings{}, ""}
			hashBlock(&genesis)
...
func requestFullBlockChain(remote string) {
	send(remote, Frame{"blockchain", host, []string{}}, func(cn net.Conn) {
		...
		for i := 0; i < numBlocks; i++ {
			blocks[i] = Block{
				frame.Data[i*4],
				Biddings{frame.Data[i*4+1], frame.Data[i*4+2]},
				frame.Data[i*4+3],
			}
		}
		blockChain <- blocks
...
func handleBlockchain(cn net.Conn) {
	...
	for i := range blocks {
		...
		frame.Data[i*4+1] = blocks[i].Payload.Ammount
		frame.Data[i*4+2] = blocks[i].Payload.Corporation
...
func addBlock(id, desc string) {
	blocks := <-blockChain
	n := len(blocks)
	newBlock := Block{blocks[n-1].Hash, Biddings{id, desc}, ""}
	hashBlock(&newBlock)
